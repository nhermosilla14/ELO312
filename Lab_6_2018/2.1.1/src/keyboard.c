...
void keyboard_button_action(char tecla)
{
  ...
  switch (tecla_real){
  case 'A':
    keyboard_set_mode(MODEA);
    break;
  case 'B':
    keyboard_set_mode(MODEB);
    break;
  case 'C':
    keyboard_set_mode(MODEC);
    break;
  case 'D':
    keyboard_set_mode(MODED);
    break;
  case '*':
    putchar('\b');
    break;
  ...
  default:
    if (tecla_real == '0'){
      if (keyboard_mode == MODED)
        putchar('0');
      else
        putchar(' ');
    } else
        if ((tecla_real + keyboard_mode) <= 90)
          putchar(tecla_real + keyboard_mode);
  }
}
